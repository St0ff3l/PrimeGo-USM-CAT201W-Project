<%@ page pageEncoding="UTF-8" %> <%-- Key: prevent garbled characters for non-ASCII text (e.g., Chinese) --%>
...
<header class="home-header <%= pgIsMerchantRoute ? " pg-header-merchant-minimal" : "" %>
  ...
  <div class="navbar">
    ...
    <%-- Icon buttons and user actions --%>
    <div class="nav-icons">
      <div class="search-container">
        <form action="${pageContext.request.contextPath}/customer/product/search_result.jsp"
      ...
      <c:if test="${not empty sessionScope.user}">
        ...
        <% if (__showMerchantTop) { %>
          ...
          <span class="pg-merchant-username" title="<%= __pgUsername %>">
            <%= __pgUsername %>
          </span>
        <% } %>

        <a href="${pageContext.request.contextPath}/profile" class="nav-avatar"
...
<style>
  ...
  .home-header *::after {
    ...
    /* Note: font-family is intentionally not set here to avoid overriding the icon font */
  }

  /* Apply font only to textual elements */
  .home-header,
  .home-header a,
  .home-header input,
  .home-header button,
  ...
  /* Force RemixIcon font for icon elements */
...
<script>
  ...
  /* About Us modal behavior */
  function openAboutUsModal(e) {
    if (e) e.preventDefault();
    const modal = document.getElementById('aboutUsModal');
    modal.style.display = 'flex'; // Ensure display is set before applying the fade-in class
    // Small timeout so the browser registers the display change before starting the opacity transition
    setTimeout(() => {
