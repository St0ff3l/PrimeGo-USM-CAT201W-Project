                ...
                <%@ page import="com.primego.product.dao.CategoryDAO" %>
                    <%@ page import="com.primego.product.model.Category" %>
                        <%@ page import="java.util.List" %>
                            ...
                            <% /* Centralized branching flags */ String pgUri=request.getRequestURI(); boolean
                                pgIsMerchantRoute=pgUri !=null && pgUri.contains("/merchant/"); User pgUser=(User)
                                session.getAttribute("user"); String pgRole=(pgUser !=null && pgUser.getRole() !=null) ?
                                pgUser.getRole().toString() : "" ; boolean pgIsMerchantUser="MERCHANT" .equals(pgRole);
                                boolean pgIsAdminUser="ADMIN" .equals(pgRole); /* Fetch Categories for Dropdown */
                                CategoryDAO __pgCategoryDAO=new CategoryDAO(); List<Category> __pgCategories =
                                __pgCategoryDAO.findAll();
                                %>

                                <link id="primego-font-poppins"
                                    href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap"
                                    rel="stylesheet">
                                <link href="https://cdn.bootcdn.net/ajax/libs/remixicon/3.5.0/remixicon.css"
                                    rel="stylesheet">
                                        ...
                                        <%-- Logo 区域 --%>
                                            ...
                                            <%-- 菜单区域 --%>
                                                <ul class="nav-menu">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <% if (__pgCategories !=null && !__pgCategories.isEmpty()) {
                                                                ...
                                                                <% for(Category c : __pgCategories) { %>
                                                                    <li><a
                                                                            href="${pageContext.request.contextPath}/customer/product/search_result.jsp?categoryId=<%=c.getCategoryId()%>">
                                                                            <%=c.getCategoryName()%>
                                                                        </a></li>
                                                                    <% } %>
                                                                        <% } else { %>
                                                    ...
                                                    <li><a href="#" onclick="openAboutUsModal(event)">About Us</a></li>
                                                    ...
                                                    <c:if test="${not empty sessionScope.user}">
                                                        <%-- admin dashboard entry --%>
                                                            <c:if
                                                                test="${sessionScope.user.role.toString() == 'ADMIN'}">
                                                                <li><a
                                                            ...
                                                            <%-- merchant dashboard entry --%>
                                                                <c:if
                                                                    test="${sessionScope.user.role.toString() == 'MERCHANT'}">
                                                                    <li><a
                                                                            href="${pageContext.request.contextPath}/merchant/merchant_dashboard.jsp">Dashboard</a>
                                                                    </li>
                                                                </c:if>
                                                ...
                                                <%-- 图标与用户操作区域 --%>
                                                    <div class="nav-icons">
                                                        <div class="search-container">
                                                            <form
                                                                action="${pageContext.request.contextPath}/customer/product/search_result.jsp"
                                                                method="get" class="search-form">
                                                                <input type="text" name="keyword"
                                                                    class="search-input-header" placeholder="Search...">
                                                                <button type="submit" class="search-btn-header"><i
                                                                        class="ri-search-line"></i></button>
                                                            </form>
                                                        </div>

                                                        <span class="nav-icon nav-icon-search" onclick="toggleSearch()"
                                                            title="Search">
                                                            <i class="ri-search-line"></i>
                                                        </span>

                                                        <span class="nav-icon nav-icon-cart"
                                                            onclick="window.location.href='${pageContext.request.contextPath}/customer/order/cart.jsp'"
                                                            title="Cart">
                                                            <i class="ri-shopping-cart-fill"></i>
                                                        </span>

                                                        <c:if test="${empty sessionScope.user}">
                                                            <a href="${pageContext.request.contextPath}/public/login.jsp"
                                                                class="nav-login-btn-link">
                                                                <button class="nav-login-btn">Login</button>
                                                            </a>
                                                        </c:if>

                                                        <c:if test="${not empty sessionScope.user}">
                                                            <% String __pgUsername=(pgUser !=null &&
                                                                pgUser.getUsername() !=null) ? pgUser.getUsername() : ""
                                                                ; boolean __showMerchantTop=pgIsMerchantRoute &&
                                                                pgIsMerchantUser; %>
                                                                <% if (__showMerchantTop) { %>
                                                                    ...
                                                                    <% } %>
                                                                        ...
                                                                        <a href="${pageContext.request.contextPath}/profile"
                                                                            class="nav-avatar"
                                                                            title="Current User: ${sessionScope.user.username}">
                                ...
                                <div id="aboutUsModal" class="about-modal-overlay" onclick="closeAboutUsModal(event)">
                                    <div class="about-modal-content" onclick="event.stopPropagation()">
                                        <button class="about-modal-close" onclick="closeAboutUsModal()">&times;</button>

                                        <h2 class="about-title">👥 About Us</h2>
                                        <div class="about-course">CAT201 Project</div>

                                        <p class="about-description">
                                            Welcome to PrimeGo, a comprehensive B2C e-commerce solution designed to
                                        ...
                                        <ul class="team-list">
                                            <li class="team-member">
                                                <span class="member-name">ZHANG ZHIQI</span>
                                                <span class="member-id">23101918</span>
                                            </li>
                                            <li class="team-member">
                                                <span class="member-name">ZHANG YIFEI</span>
                                                <span class="member-id">23101912</span>
                                            </li>
                                            <li class="team-member">
                                                <span class="member-name">ZHANG JUN</span>
                                                <span class="member-id">23101903</span>
                                            </li>
                                            <li class="team-member">
                                                <span class="member-name">CHEN ZEKAI</span>
                                                <span class="member-id">23101653</span>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <script>
                                    ...
                                    function openAboutUsModal(e) {
                                        ...
                                        modal.style.display = 'flex'; // Ensure display is flex before adding active class for opacity
                                    ...
                                    function closeAboutUsModal(e) {
                                        if (e && e.target !== e.currentTarget && !e.target.classList.contains('about-modal-close')) return;
